...
const setAuthCookies = (res, accessToken, refreshToken) => {
	// Set the cookie
	if (accessToken != null) {
		logger.trace("Setting authorization cookie.");
		res.cookie("authorization", accessToken, {
			httpOnly: true,
			secure: process.env.NODE_ENV === "production",
			maxAge: 1000 * 60 * 25, // *About* 15 minutes (in milliseconds)
		});
	} else logger.debug("Found null access token when setting cookie.");
	if (refreshToken != null) {
		logger.trace("Setting refresh token cookie.");
		res.cookie("refreshToken", refreshToken, {
			httpOnly: true,
			secure: process.env.NODE_ENV === "production",
			maxAge: 1000 * 60 * 60 * 24 * 3, // *About* 3 days (in milliseconds)
		});
	} else logger.debug("Found null refresh token when setting cookie.");
};
