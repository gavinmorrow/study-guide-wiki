...
const mapUserDbToClass = async user => {
	...
	try {
		...
		const guideAccess = await db.any("SELECT * FROM guide_access WHERE user_id = $1", [
			user.id,
		]);
		...
		/** @type {string[]} The guide ids */
		const ownedGuides = (
			await db.any("SELECT (id) FROM guides WHERE owner_id = $1", [user.id])
		).map(guide => guide.id);
...
const users = {
	 ...
	 */
	async get(id) {
		if (id == null) return logger.error("Attempted to get user with null id");
	...
	async getId(displayName) {
		if (displayName == null) return logger.error("displayName cannot be null");

		try {
			const user = await db.oneOrNone(`SELECT id FROM users WHERE display_name = $1`, [
				displayName,
			]);

			if (user == null) return logger.debug(`User "${displayName}" not found`);
	...
	async displayNameIsUsed(displayName) {
		const user = await db.oneOrNone("SELECT * FROM users WHERE display_name = $1", [
			displayName,
		]);
	...
	async add(user) {
		try {
			await db.none("INSERT INTO users (id, password, display_name) VALUES ($1, $2, $3)", [
				user.id,
				user.password,
				user.displayName,
			]);
